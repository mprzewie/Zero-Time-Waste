...
class BasicBlockWOutput(nn.Module):
    ...
    def __init__(
            self, args, in_channels, channels, params,
            stride=1, head_variant=None, heads_per_ensemble=1,
            prev_dim=0):
        ...
        if add_output:
            self.output = af.InternalClassifier(args,
                                                ...
                                                heads_per_ensemble=heads_per_ensemble,
                                                prev_dim=prev_dim)
    ...
    def forward(self, x, prev_output=None):
        ...
        return self.layers[2](fwd), 1, self.output(fwd, prev_output)  # output layers for this module
